...
if __name__ == "__main__":
    ...
    print("\n--- Sanity Check: q-vector at Direct Beam Center ---")
    # Use the beam and detector from the loaded experiment file (e.g., indexed.expt)
    # test_beam_model and test_detector_model are already loaded
    ...
    if test_detector_model:
        ...
        try:
            beam_center_mm_fast, beam_center_mm_slow = test_beam_model.get_beam_centre(panel_for_beam_test)
            print(f"  Beam center on Panel {panel_for_beam_test_idx} (dxtbx): ({beam_center_mm_fast:.2f}, {beam_center_mm_slow:.2f}) mm from panel origin")
            ...
            # Note: dxtbx beam_centre is in mm in the panel frame.
            # Our pixel indexing convention:
            # dist_fast_mm = (px_idx + 0.5) * pixel_size_fast
            # px_idx = (dist_fast_mm / pixel_size_fast) - 0.5
            ...
            print(f"  Calculated beam center pixel (fast, slow): ({beam_px_f_idx}, {beam_px_s_idx})")
            ...
            if not (0 <= beam_px_f_idx < num_f_panel and 0 <= beam_px_s_idx < num_s_panel):
                ...
                q_at_beam_center = np.array([
                    qbm_qx[beam_px_s_idx, beam_px_f_idx],
